...
namespace AudioUtils 
{
	...
	public class WaveControl : System.Windows.Forms.UserControl
	{
        ...
        /// <summary>
        /// Boolean for whether the user clicked on the graph and a line should be draw
        /// </summary>
        private bool m_DrawLine = false;
        private int m_LinePoint = 0;
        private int m_maxWidth = 0;

        private DSPMediaPlayer.Audio.WaveControlCB m_CBFunc;


        public DSPMediaPlayer.Audio.WaveControlCB CBFunc
        {
            set { m_CBFunc = value; }
        }
		...
		private void InitializeComponent()
		{
            ...
            this.MouseClick += new System.Windows.Forms.MouseEventHandler(this.WaveControl_MouseClick);
		...
		private void WaveControl_Paint(object sender, System.Windows.Forms.PaintEventArgs e)
		{
			...
			//in case a user selected a region ==> we'll cancell the line draw
            if ((0 != regionEndX)
                && (regionStartX != regionEndX))
            {
                m_DrawLine = false;
            }

			//if we need to draw a line ==> draw it.
            if (true == m_DrawLine)
            {   
                e.Graphics.FillRectangle(brush, m_LinePoint, 0, 1, e.Graphics.VisibleClipBounds.Height);
            }

			//saving the max window width
            m_maxWidth = (int)e.Graphics.VisibleClipBounds.Width;
		...
		private void PerformScroll( int newXValue )
		{
        ...
        private void ZoomOutFullButton_Click(object sender, EventArgs e)
        {
            ZoomOutFull();
            Refresh();
        }
        ...
        private void WaveControl_MouseClick(object sender, MouseEventArgs e)
        {
			//tell the draw function to draw a line
            m_DrawLine = true;
			
			//save the draw line point
            m_LinePoint = e.X;
        ...
        public void GetXCoordinatesinPercentage(ref int arStartLocation, ref int arEndLocation)
        {
            double temp = 0;
            int regionStartX = Math.Min(m_StartX, m_EndX);
            int regionEndX = Math.Max(m_StartX, m_EndX);

			//check if the user pin point a line (and not a region)
            if(     (regionStartX == regionEndX)
                || (0 == regionEndX))
            {
                temp = (m_OffsetInSamples + m_LinePoint * m_SamplesPerPixel) * 100 / m_Wavefile.Data.NumSamples;
                arStartLocation = (int)temp;

				//at this case the end if the end of the window
                arEndLocation = 100;
                return;
            }

			//this point means: user selected a section
            temp = (m_OffsetInSamples + regionStartX * m_SamplesPerPixel) * 100 / m_Wavefile.Data.NumSamples;
            arStartLocation = (int)temp;

            temp = (m_OffsetInSamples + (regionEndX - regionStartX) * m_SamplesPerPixel) * 100 / m_Wavefile.Data.NumSamples;
            arEndLocation = (int)temp;

           }

        public void GetInfo(ref int aNumSamples, ref double aCurrentCurserInMs, ref double aUserSelectionDurationInMs)
        {
            int regionStartX = Math.Min(m_StartX, m_EndX);
            int regionEndX = Math.Max(m_StartX, m_EndX);

            aNumSamples = m_Wavefile.Data.NumSamples;

            aCurrentCurserInMs = ((m_OffsetInSamples + m_LinePoint * m_SamplesPerPixel));


            aUserSelectionDurationInMs = (regionEndX - regionStartX) * m_SamplesPerPixel;
        }
	}
}
